...
func (b *DBroke[E, T]) SetBlock(data string) *DBroke[E, T] {
	b.data.BlockHash = unsafeConvert.StringSlice(hmac.Shake256S(litefmt.PSprint("0x", data), 256))
	return b
}

func (b *DBroke[E, T]) SetLotteryID(data string) *DBroke[E, T] {
	b.data.LotteryID = unsafeConvert.StringSlice(hmac.Shake128S(data, 64))
	return b
}
